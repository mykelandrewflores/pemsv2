...
$('#form_request').submit(function (e) {
    ...
    if (cpwd == cpwd_t) {
        ...
        fetch(myurls + "pems/apis/myapi/insert/tbl_companies",
            {method:"POST",
            body: JSON.stringify([{
                cn: cname,
                ca: cadd,
                ...
                swal("Register Successfully", "We've already sent an email for your verification code please check your email now", "success").then(function () {
                    setTimeout(function () {
...
$('#login_user').submit(function (e) {
    ...
    $.ajax({
        beforeSend: function () {
            ...
            document.querySelector('#btn_submit').innerHTML = 'Loging in...';
        },
        url: myurls + "pems/apis/myapi/login/",
        type: "POST",
        data: {
            un: un,
            pw: pw
        ...
        document.querySelector('#btn_submit').innerHTML = 'Signin';
        $('#login_user')[0].reset();
        if (data.Authorize == "User") {
            ...
            swal("Failed Login", "No Account Found either wrong username and password", "error");
        }
    });
});
...
function verifyCode() {
    ...
    if (val != "") {
        $.getJSON(myurls + "pems/apis/myapi/select/tbl_companies/fldVerify/" + val, function (data) {
            if (data.length > 0) {
                updateData(val);
                swal("Verify Successfully", "Account already verify please login now!", "success").then(function () {
                    window.location.assign("index.html");
                });
            } else {
                swal("Invalid!", "Invalid Verification Code. Resend or Check to your email address", "error");
            }
        });
    } else {
        swal("Enter a Code", "Fields are required", "error");
    }
}
...
function resetPass() {
    ...
    setTimeout(function () {
        if ($("#email").val() != "") {
            ...
            swal("Send Succesfully", "Check your email!", "success").then(function () {
                window.location.assign("verify.html");
            });
        } else {
            swal("Fields Required!", "Field are required", "error");
        }
    }, 1000);
