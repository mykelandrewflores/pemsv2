...
function showTable() {
    ...
    for (let i = 0; i < yearlyCost.length; i++) {
        ...
        for (let k = 0; k < yearlyCost[i].Items.length; k++) {
            document.getElementById("itemNo" + (k + 1)).innerHTML += "<td>" + yearlyCost[i].Items[k].TotalCost;
            ArrTotal[k] += parseFloat(yearlyCost[i].Items[k].TotalCost);
        }
    }
...
function showChart() {
    ...
    lowestkey = 0;
    lowestval = parseFloat(ArrTotal[0]);
    for (let j = 0; j < itemCount; j++) {

        if (lowestval > parseFloat(ArrTotal[j])) {
            lowestval = parseFloat(ArrTotal[j]);
            lowestkey = j;
        }

        document.getElementById("itemNo" + (j + 1)).innerHTML += "<td><strong>" + ArrTotal[j] + "</strong></td>";
    }
        ...
        for (let j = 0; j < datas[3].items.length; j++) {
            ...
            for (let k = 0; k < costUsed.length; k++) {
                ...
                if (parsedJSON.costType == "EscalationValue" && (i + 1) > 1) {
                    escalatedVal = (((parseFloat(parsedJSON.EscalationValue).toFixed(2) / 100) * parseFloat(parsedJSON.costValue)) * i) + parseFloat(parsedJSON.costValue);
